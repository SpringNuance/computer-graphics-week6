...
float D(vec3 N, vec3 H) {
	  float cos = dot(normalize(N), normalize(H));
	  float tan = sqrt(1-pow(cos, 2))/cos;
	  float sqrRough = pow(roughness, 2);
	  float temp = pow(sqrRough + pow(tan, 2), 2);
	  float D = sqrRough/(3.14159 * pow(cos, 4) * temp);
	  return D;
}
...
float G1(vec3 X, vec3 H) { 
	float sqrRough = pow(roughness, 2);
    float cos = dot(normalize(X), normalize(H));
    float tan = sqrt(1-pow(cos, 2))/cos;
	float G = 2/(1 + sqrt(1 + sqrRough*pow(tan,2)));
	return G;
}
...
float Fr(vec3 L, vec3 H) {
	...
	float eta = n1/n2;
	float sqreta = pow(eta, 2);
	float cos = dot(normalize(L), normalize(H));
	float sqrcos = pow(cos, 2);
	float beta = sqrt(1/pow(eta, 2) + sqrcos - 1);
	float Rs = pow((cos - beta)/(cos + beta),2);
	float Rp = pow((sqreta * beta - cos)/(sqreta * beta + cos), 2);
	float Fr = 0.5f * (Rs + Rp);
	return Fr;
}
...
void main()
{
	...
	for (int i = 0; i < numLights; ++i)
	{
		...
		vec3 diffuse = diffuseColor.rgb * max(0.0f, dot(N, L)) * Li;
		light_contribution += diffuse;
		

		// YOUR CODE HERE (R3, R4, R5)
		// Compute the GGX specular contribution of this light.
		vec3 specular = specularColor.rgb * CookTorrance(N, H, V, L);
		light_contribution += specular * max(0.0f, dot(N, L)) * Li;
